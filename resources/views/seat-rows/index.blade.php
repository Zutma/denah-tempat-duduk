...
@section('content')
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-5 mb-6">
        ...
        <form method="POST" action="{{ route('sessions.seats.store', $session) }}" class="grid grid-cols-1 sm:grid-cols-4 gap-4 items-end">
            @csrf
            <div>
                <label for="row" class="block text-xs font-medium text-gray-700 mb-1">Nama Baris (1 huruf)</label>
                <input type="text" id="row" name="row" maxlength="1" value="{{ old('row') }}" placeholder="A"
                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm uppercase focus:ring-2 focus:ring-sky-500 outline-none" required>
            </div>
            <div>
                <label for="side" class="block text-xs font-medium text-gray-700 mb-1">Sisi</label>
                <select id="side" name="side" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-sky-500 outline-none">
                ...
                @forelse ($seatRows as $seatRow)
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-center">
                            <span class="inline-block px-2.5 py-0.5 text-xs font-semibold rounded-md border {{ $seatRow->side == 'left' ? 'bg-blue-50 text-blue-700 border-blue-200' : 'bg-purple-50 text-purple-700 border-purple-200' }}">
